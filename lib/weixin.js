  ...
  if (fs.existsSync(but_json_path)) {
    ...
    if (!file_info.isDirectory()) {
      ...
      if (token_result.success) {
        ...
        //let btn_url = `https://open.weixin.qq.com/connect/oauth2/authorize?appid=${config.weixin_info.appID}&redirect_uri=${encodeURI('http://tsml520.cn/wx/he_live')}&response_type=code&scope=snsapi_userinfo&state=params#wechat_redirect`;
        //btn_json = `${btn_json}`;
        //btn_json = btn_json.replace('__url__', btn_url);
...
* 根据code获取用户token信息
*/
async function get_wx_user_token(code){
  let url = `/sns/oauth2/access_token?appid=${config.weixin_info.appID}&secret=${config.weixin_info.appSecret}&code=${code}&grant_type=authorization_code`;
  let result = await httpsGet('api.weixin.qq.com','443',url,{});
...
* 根据code获取用户信息
*/
async function get_wx_user_info(token, appID){
  console.info(token, appID);
  let url = `/sns/userinfo?access_token=${token}&openid=${appID}&lang=zh_CN`;
  let result = await httpsGet('api.weixin.qq.com','443', url, {});
  return JSON.parse(result);
}
...
*/
async function get_wx_user_info_by_openid(token,openid){
  let url = `/cgi-bin/user/info?access_token=${token}&openid=${openid}&lang=zh_CN`;
  let result = await httpsGet('api.weixin.qq.com','443', url, {});
  return result;
}

export { get_wx_user_token, get_wx_user_info, get_wx_user_info_by_openid, get_wx_token }
